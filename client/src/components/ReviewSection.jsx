...
export default function ReviewSection({ targetModel, targetId }) {
  ...
  useEffect(() => {
    ...
    getMyRentalReviewWriteEligibility(targetModel, targetId)
      .then(({ data: d }) => {
        if (!cancelled) setRentalWriteElig(d);
      })
      .catch(() => {
  ...
  const rentalHint = () => {
    if (!rentalWriteElig || rentalWriteElig.eligible) return "";
    if (rentalWriteElig.code === "no_rental") {
      return fr
    ...
    if (rentalWriteElig.code === "before_last_day") {
      return fr
    ...
    return fr ? "Impossible de vérifier les droits d’avis." : "Could not verify review eligibility.";
  };

  const showWriteForm = auth?._id && (targetModel !== "RentalListing" || (rentalWriteElig && rentalWriteElig.eligible));
  ...
  return (
    <div style={{ marginTop: 32 }}>
      ...
      {auth?._id && targetModel === "RentalListing" && rentalWriteElig === undefined ? (
        <p style={{ fontSize: 13, color: "#9ca3af", marginBottom: 16 }}>{fr ? "Vérification…" : "Checking eligibility…"}</p>
      ) : null}

      {auth?._id && targetModel === "RentalListing" && rentalWriteElig && !rentalWriteElig.eligible ? (
        <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 20, lineHeight: 1.5 }}>{rentalHint()}</p>
      ) : null}
      ...
      {showWriteForm ? (
        ...
        <p style={{ fontSize: 13, color: "#6b7280", marginBottom: 20 }}>
          <a href="/login" style={{ color: "#3d3af5", fontWeight: 600 }}>
            Log in
          </a>{" "}
          to leave a review.
        </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {data.reviews.map((r) => (
            ...
            >
              ...
              {r.comment && <p style={{ margin: 0, fontSize: 13, color: "#4b5563", lineHeight: 1.6 }}>{r.comment}</p>}
