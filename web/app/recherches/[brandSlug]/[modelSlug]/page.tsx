...
import SearchIntelModelView, { searchIntelMetadata } from "@/lib/views/SearchIntelligenceView";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const meta = searchIntelMetadata("fr", brandSlug, modelSlug);
  if (!meta) return {};
...
export default async function Page({ params }: Props) {
  const { brandSlug, modelSlug } = await params;
  if (!searchIntelMetadata("fr", brandSlug, modelSlug)) notFound();
  return <SearchIntelModelView lang="fr" brandSlug={brandSlug} modelSlug={modelSlug} />;
}
