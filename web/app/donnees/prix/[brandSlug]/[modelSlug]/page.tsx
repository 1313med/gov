...
import DatasetView, { datasetMetadata } from "@/lib/views/DatasetView";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const meta = datasetMetadata("fr", brandSlug, modelSlug);
  if (!meta) return {};
...
export default async function Page({ params }: Props) {
  const { brandSlug, modelSlug } = await params;
  if (!datasetMetadata("fr", brandSlug, modelSlug)) notFound();
  return <DatasetView lang="fr" brandSlug={brandSlug} modelSlug={modelSlug} />;
}
